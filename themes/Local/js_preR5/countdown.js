...
if(!Array.prototype.indexOf)Array.prototype.indexOf=function(c){
    ...
    if(i>=a)return-1;
    for(i=i>=0?i:Math.max(a-Math.abs(i),0);i<a;i++)if(i in b&&b[i]===c)return i;return-1
    };
...
c.contains=function(a,b){
    ...
    g.xMax=g.x+g.w;
    g.yMax=g.y+g.h;
    for(var f in e)c[f]=e[f]>=g[f]?true:false;return!c.x&& !c.y&&c.xMax&&c.yMax?true:false
    };
...
var CountdownImageFolder="../js/images/",CountdownImageBasename="flipper",CountdownImageExt="png",CountdownImagePhysicalWidth=41,CountdownImagePhysicalHeight=60,CountdownWidth=200,CountdownHeight=30;
(function(){
    var c=function(a){
    ...
    for(d=0;d<b.length;d++)if(d>=f&&d<=h)this.bq[b[d]].use=true;e=a.padding===0?0:a.padding?a.padding:this.T=="flip"?0:0.8;
    this.T=="flip"&&(e/=2);
    ...
    if(a.numbers)for(var n in p)a.numbers[n]&&(p[n]=a.numbers[n]);if(a.labels)for(n in w)a.labels[n]&&(w[n]=a.labels[n]);b.reverse();
    this.aN={};
