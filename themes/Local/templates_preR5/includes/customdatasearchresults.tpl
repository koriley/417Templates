<style type="text/css">
<!--
@import url("/media/css/{$table.tabletype}-data.css");
-->
</style>
...
<table width="100%"  border="0" cellpadding="5" cellspacing="0" id="customdata-search-table">
   <tr class="header">
		...
		<td height="25" class="header"><a href="{sortlink field="additionalinfo"}" class="sort">Cuisine<img align="absmiddle" src="{$images}/order-{if $Page->params.sort=="additionalinfo"}{if $Page->params.so=="asc"}desc{else}asc{/if}{else}desc{/if}.gif" alt="Sort {if $Page->params.sort=="additionalinfo"}{if $Page->params.so=="asc"}Descending{else}Ascending{/if}{else}Descending{/if}" width="48" height="15" border="0" /></a></td>
		<td height="25" class="header"><a href="{sortlink field="areaname"}" class="sort">Area<img align="absmiddle" src="{$images}/order-{if $Page->params.sort=="areaname"}{if $Page->params.so=="asc"}desc{else}asc{/if}{else}desc{/if}.gif" alt="Sort {if $Page->params.sort=="areaname"}{if $Page->params.so=="asc"}Descending{else}Ascending{/if}{else}Descending{/if}" width="48" height="15" border="0" /></a></td>	
	</tr>
