...
function doFormPost(formname,pressbutton)
{
   var msg="";
	...
	return autocheck(eval('window.document.'+formname));
...
<table width="600" border="0" cellspacing="0" cellpadding="5">
  <tr>
    <td width="320"><table width="100%"><tr><td>
<img src="{$publication.url}image.php" width="156" height="202"> </td></tr></table></td>
    <td valign="top"><P class=style4>We are excited that you have chosen to renew your 
      subscription to our magazine. </P>
      {if $Page->user->id==''}
      <P class=style4>If you have an account, you may <strong>login</strong> and we will retrieve your information for faster renewal processing.</p> <p> <i>If you do not have an account, please skip login and proceed below.</i></P></td>
  </tr>
</table> <p></p>
      
<FORM id="login" name="login" method="post"><p><strong>Login:</strong></p>
      <TABLE height=40 cellSpacing=0 cellPadding=0 width="75%" border=0>
        ...
        <TR>
          ...
          <TD><INPUT id="login_password" type=password name="login_password" label="Login Password" onblur="requireValue(this)"></TD></TR>
        <TR>
		...
		{/if}
	      ...
	      <p>Please fill out the following information so that we may process your renewal. <i><font color="#FF0000">(items with * are required)</font></p></i>
      
<FORM id="order" name="order" method="post"  onsubmit="autocheck(this)">
