...
var ModuleEventExchange = (function() {
	...
	return {
		...
		triggerEvents: function(event) {
			jQuery.each(ModuleEventExchange.eventExchange[event.type], function(index, eventType) {
				jQuery(eventType[0]).trigger(eventType[1]);
			});
		},
		...
		register: function(parameters) {
			...
			if (parameters.detectedEvent != null && parameters.triggeredEvent != null) {
				...
				if (typeof ModuleEventExchange.eventExchange[parameters.detectedEvent] != "undefined") {
					...
					jQuery.each(ModuleEventExchange.eventExchange[parameters.detectedEvent], function(index, trigger) {
						if (jQuery(newTrigger).not(trigger).length == 0) {
			...
			jQuery("body").on(parameters.detectedEvent, function(event) {
...
jQuery(document).ready(function() {
	jQuery.each(ModuleEventExchange.eventExchange, function(listener, targets) {
		jQuery("body").on(listener, function(event) {
			ModuleEventExchange.triggerEvents(event);
		});
	});
});	
