...
<link href="/themes/Local/css/department-archive-bvm.css" rel="stylesheet" type="text/css" />
...
{include file="top-bvm4.tpl"}
	<div id="page-body">
		...
		{section name=crumb loop=$breadcrumbs} {strip} {if $smarty.section.crumb.last} {$breadcrumbs[crumb].title} - {$taglist[t].title} {else} <a href="{$breadcrumbs[crumb].url}" class="breadcrumblink">{$breadcrumbs[crumb].title}</a>&nbsp;/&nbsp;{/if} {/strip} {/section}
	 	...
	 	<div id="columns-1-2">
		<div class="header-silver"><h6 class="large">The Must List - {$taglist[t].title}</h6></div>
		<div id="column-1">
