        ...
        <div class="row-fluid">
			...
			<article class="article page-content">
					...
					{if $article.author<>''}<div class="by-line"><!--{$strings_article_template_byline_intro}-->{$article.author}</div>{/if}
                                 ...
                                 <p style="font-size:80%"><em><strong>DISCLAIMER:</strong> The information in this article was fact checked and accurate at press time, but </em>417 Magazine<em> cannot guarantee its accuracy indefinitely.</em></p>
...
{literal}
    <script>
       var imagePath = jQuery('.faceBookImage img').attr("src");
      //  console.log(imagePath);
        jQuery('meta[name=fbimage]').remove();
